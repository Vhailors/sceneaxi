...
| Credits + billing | `packages/billing` (append-only credit ledger, metering, free-vs-paid enforcement, default-off hosted-AI credit gate, Stripe test-mode checkout, signed-webhook grants and full-refund reconciliation, creator revenue share, the bounded fixture-SKU commerce path over the committed catalog, the TEST-only Stripe Connect creator onboarding and payout-bookkeeping seam described in [`docs/stripe-connect-operations.md`](docs/stripe-connect-operations.md)) |
...
models each site renders through a thin React adapter — also lives in
`packages/site-kit` and is owned by
