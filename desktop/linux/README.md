...
hierarchy v1: stable object ids, parent/child depth, ordered multi-selection,
bounded local-artifact create/remove, and explicit preserve-world or
preserve-local reparenting. These commands use the same Change Review,
transaction history, reopen, and Play paths; the CLI and local assistant inspect
the same hierarchy through the permission-bound desktop bridge. The focused
end-to-end proof is `tests/e2e/desktop-hierarchy-golden.test.ts`; the Electron
smoke above remains the packaged-host proof and does not claim the later #254
gizmo or #258 Play-lifecycle work.
