...
| `@sceneaxi/authoring-core` | `./src/index.ts` | `./desktop-session-authority` (desktop-session-owned, root-bound Git mutation binding; no authority token or mutation function is re-exported from the SDK root), `./rarity-evidence` (the shared safe-rarity-provenance formatter on an import-free module, so a browser bundle can reach it without the Node-bearing root barrel) |
