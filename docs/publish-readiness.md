...
**Scope:** this is the repository-wide record — every workspace package, and the gate
that enforces it. The public engine SDK archive carries its own copy of this document,
generated by `scripts/build-engine-sdk.mjs` from the manifests that archive ships and
scoped to them, so an outsider reads a statement about what they actually have. It is a
subset of this one, never a different one.
