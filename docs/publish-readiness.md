...
| `@sceneaxi/auth` | `./src/index.ts` | `./testing/*` (test-only issuance seam for genuine principal fixtures, never re-exported from the root barrel; the boundary checker refuses every package, app, and site source file that imports it) |
...
| `exports-resolve` | Every `exports` target is an explicit, real, non-symlink file inside its own package — no dangling public entry point, and no subpath pattern, which could be proven neither to resolve nor to ship. Every workspace manifest outside the `sites/` tier must declare one; a deployable site need not. |
...
| `internal-deps-workspace` | Every internal `@sceneaxi/*` dependency of every manifest, root included, uses the `workspace:` protocol — except a deployable site, which is its own install root and uses a `link:` path into `packages/` (ADR 0018). Never `file:`, a Git ref, or a version range. |
