...
|---|---|---|
| `view` | changes visual state; genuinely works | mode rail, dock tabs, profile switch, assistant open/close, its Light/Mid/Strong strengths and its three present-but-hidden route chips, menu openers, the palette openers, the outcome dismissal, drawer toggles, and the hierarchy multi-select |
...
| Kids profile | a working editor with only the assistant locked | the **whole editor body** refuses, restyled as a cheerful Kids studio | **contract conflict, resolved for the repository.** Kids authoring exists only on its dedicated, simplified origin (`kids-first-release.md`); the shared desktop open path remains `OPEN_PATH_KIDS_REFUSED`. The captain asked for a colorful assistant-only Kids chrome; this surface now presents that studio copy, but it still does not author a scene and still does not send Kids traffic to OpenCode or any third-party model. Every control behind the refusal goes inert — in the emitted bytes, not only after a click — so no Game or Website tool can be entered from behind it; the profile switch stays live so the refusal is a state you can leave. |
...
| Assistant product flow (sceneaxi#192) | nothing recorded: this document has never carried an assistant composer inventory from the archive, and the archive is a design input for visual values, not for product flow | a real prompt `<textarea>`, Light / Mid / Strong strengths, a Send action, an OpenCode Flash key field, a live busy theater (`data-assistant-busy`) with pulse/bars while work runs, progress and result regions that report actual work, a `Retry` action, and a viewport manipulator bar. Website chrome is a page studio (`example.com` hero + page sections), not the Game object inspector. Route chips stay present-but-hidden. | **product decision, not a visual one.** Strength is the only headline choice; Flash is the model. The busy theater is decorative and does not invent progress. Send is `live` only after the packaged host binds. Kids still cannot reach a third-party model. Website authoring stays page/HTML/site-canvas/Three embed. |
