...
Issues #250–#252 now supply the command-registry, native-project, and transaction
slices; #263 supplies the contained Git slice described below. The rows name only
their evidence and do not claim capabilities owned by other issues. The matrix
reports current behavior. A `live` control in the visual model is not
...
The shared shell model owns **90 unique control ids** across all modes and runtime
projections. A default Game or Web render contains 89 because `dock-timeline`
...
counted as shell controls, so the current packaged-window union is **94**.
...
the same-user local agent bridge exposes **23 tools** from
...
| `menu-command-{project-new,project-open,project-save,project-git-status,project-git-diff,ship-export-web,edit-undo,edit-redo,run-play}` | 9 | **real** | Each invokes the same current handler as its palette row and accelerator through the registered desktop-control contract; native New/Open keep the lifecycle port while the other engine-host operations cross the validated `command` action. Git status/diff render the host's shared repository evidence in Ship without writing; undo and redo are visibly inert until durable history reports availability or while recovery is pending. | command interaction golden; contained Git golden; full-editor transaction golden; desktop bridge golden | None for this slice. |
...
| `palette-{project-new,project-open,project-save,project-git-status,project-git-diff,ship-export-web,edit-undo,edit-redo,run-play}` | 9 | **real** | Invoke the same registered commands as menu rows; Git evidence is the same value returned to CLI and local-agent clients, while unavailable document-history commands preserve focus and name the refusal. | command interaction golden; contained Git golden; full-editor transaction golden | None for this slice. |
...
| CLI | `desktop bridge call`, `status`, `tools` | **real** | Discovers and authenticates the same-user Unix socket; validates exact tool, permission, and input. `tools` exposes the same command schema version and definitions; registered calls include project inspection, migration, and contained Git status/diff/stage/commit-preparation without a second CLI result model. | CLI bridge, local RPC, contained Git, and CLI→local bridge goldens. |
...
| Desktop interaction table | New, Open, Save, Git Status, Git Diff, Export Web, Undo, Play | **real** | All eight derive identity, label, schema version, and permission from the shared registry; Git inspection displays the shared repository evidence, and menu, palette, button, and accelerator parity remains executable. | `apps/desktop-shell/src/interaction-commands.ts`; command interaction and contained Git goldens. |
...
| Local-agent tools | handshake; project status/inspect/propose/accept/reject/recover/restart/undo/redo, migration propose/commit/recover, and Git status/diff/stage/commit-prepare; Play; assistant Local start/BYOK start/bounded Agent/status/abandon | **partial** | Twenty-three real permission-bound tools. Git mutations accept only explicit contained paths, and UI (`desktop-control`), CLI, and local-agent paths receive the same repository state/evidence value. The transport still excludes credentials, Hosted, renderer-only frame actions, push, fetch, and history operations. | registry and desktop local bridge contracts; contained Git and full-editor transaction goldens; CLI→local bridge golden. |
...
| Contained Git workflow | **real** | `project-git-{status,diff,stage,commit-prepare}` share one project-root service and typed repository evidence across desktop, CLI, and local-agent clients. Status/diff report canonical manifest-owned files plus unrelated changes without writes; stage names exact paths; preparation requires the complete staged selection and creates no commit. Named pre-mutation refusals cover SceneAxi review/recovery/transaction state, conflicts, detached worktrees, missing Git/capability, Kids, path/repository escape, and unsupported push/fetch/history/branch deletion. Document undo/redo remains explicitly `sceneaxi-document-only`. | `packages/authoring-core/test/project-git.test.ts`; `tests/e2e/contained-git-golden.test.ts`; command interaction golden | None for #263; no remote or history-mutating Git surface is authorized. |
