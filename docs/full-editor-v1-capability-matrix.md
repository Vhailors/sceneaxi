...
| `scene-property-stage` | 1 | **partial** | Builds one `/data/composedScene` proposal against the observed content hash. | Same as transform fields | [#254](https://github.com/Vhailors/sceneaxi/issues/254). |
...
| `dock-console` | 1 | **fake** | The tab works as presentation, but it always says no session is running and owns no log stream. | `apps/desktop-shell/src/chrome.ts` | [#258](https://github.com/Vhailors/sceneaxi/issues/258) supplies a runtime session. |
...
| CLI | `desktop bridge call`, `status`, `tools` | **real** | Discovers and authenticates the same-user Unix socket; validates exact tool, permission, and input. `tools` exposes the same command schema version and definitions; registered calls include project migration, contained Git, and hierarchy inspect/select/create/remove/reparent without a second CLI result model. | CLI bridge, local RPC, contained Git, hierarchy, and CLI→local bridge goldens. |
...
| Desktop interaction table | New, Open, Save, Git Status, Git Diff, Git Stage, Prepare Git Commit, Export Web, Undo, Redo, Play | **real** | All eleven derive identity, label, schema version, and permission from the shared registry; Git inspection displays the shared repository evidence, and menu, palette, button, and accelerator parity remains executable. | `apps/desktop-shell/src/interaction-commands.ts`; command interaction and contained Git goldens. |
...
| Local-agent tools | handshake; project status/inspect/propose/accept/reject/recover/restart/undo/redo, migration propose/commit/recover, and Git status/diff/stage/commit-prepare; hierarchy inspect/select/create/remove/reparent; Play; assistant Local start/BYOK start/bounded Agent/status/abandon | **partial** | Twenty-eight real permission-bound tools. Git mutations accept only explicit contained paths, and UI (`desktop-control`), CLI, and local-agent paths receive the same repository state/evidence value. The transport still excludes credentials, Hosted, renderer-only frame actions, push, fetch, and history operations. | registry and desktop local bridge contracts; contained Git, full-editor transaction, and hierarchy goldens; CLI→local bridge golden. |
