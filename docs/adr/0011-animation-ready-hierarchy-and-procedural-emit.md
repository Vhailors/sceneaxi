...
generated artifacts. Given a sculpt-quality multi-pass spec and a non-negative
JavaScript safe-integer seed, it emits canonical geometry detail, material
detail, hierarchy parentage, and deterministic transforms. The artifact records
the implementation-program digest, seed, and emit digest; validation
independently recomputes the emit from the spec and seed.
