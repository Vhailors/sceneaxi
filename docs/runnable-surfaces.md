...
| `@sceneaxi/desktop-shell` | **R2** | `pnpm build && node apps/desktop-shell/bin/sceneaxi-desktop.mjs --help`; for standalone chrome, `… chrome > shell.html`; the packaged host activates New, Open, Save, Undo, Play, and Export Web through the existing menus/palette and the Ship rail, plus the contained project/asset browser, selected-instance transforms, and local add/remove | `apps/desktop-shell/test/bin-smoke.test.ts`, `apps/desktop-shell/test/{product-loop,visual-model,visual-tokens,chrome}.test.ts`, `tests/e2e/desktop-product-loop-golden.test.ts`, `tests/e2e/desktop-project-browser-golden.test.ts`, `tests/e2e/desktop-command-interactions-golden.test.ts`, `tests/e2e/desktop-web-export-golden.test.ts`, `tests/parity/shell-cli-parity.test.ts` + the browser record in [`engine-desktop-surface.md`](engine-desktop-surface.md) |
