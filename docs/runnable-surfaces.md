...
| `@sceneaxi/desktop-linux` (packaged Linux desktop app, ADR 0024) | **R2** | `cd desktop/linux && pnpm install && pnpm build && pnpm start`; first launch offers contained New/Open/Recent project lifecycle; a bound root lists its canonical `scene.json` and admitted manifest assets, opens supported selections, and keeps canonical rows immutable; Web Experience exposes native contained GLB/glTF import through E1 Change Review and project-owned copy; a bound project exposes hierarchy v1, ordered multi-select, explicit-policy parenting, primary-object translation/rotation/scale, and validated-local-artifact create/remove, each staged as one E1 proposal and written only on Save; Ship exports a deterministic contained static Web viewer plus Delivery Handoff without deploying; Assistant Build's Local route compiles and mounts a typed sculpt without a provider; the running host also publishes the permission-bound protocol-v1 local CLI bridge; distributable via `pnpm dist` (AppImage + `.deb` + `SHA256SUMS`), proven by `pnpm smoke --packaged` | `tests/desktop/desktop-project-browser.test.ts` and `tests/e2e/desktop-project-browser-golden.test.ts` (containment, metadata, selection recovery, UI Open, and immutable rename/delete refusals), `tests/e2e/asset-ingestion-golden.test.ts` + `tests/desktop/desktop-asset-picker-host.test.ts`, `tests/e2e/desktop-web-export-golden.test.ts` (byte-stable output, pinned digests, handoff verification, refusals), `tests/e2e/desktop-project-lifecycle-golden.test.ts` (preload-shaped create/open/cancel/recent/restart interaction), `tests/desktop/desktop-project-lifecycle.test.ts` (root, symlink, migration, invalid-byte, atomic-state contract), `tests/e2e/desktop-linux-bridge-golden.test.ts` (assistant → typed artifact → headless mount/manipulator contract, no pixel claim), `tests/e2e/desktop-cli-local-bridge-golden.test.ts` (spawned CLI → Unix socket → shared authoring session and hierarchy inspection), `tests/desktop/desktop-scene-property.test.ts`, `tests/e2e/desktop-scene-property-golden.test.ts`, and `tests/e2e/desktop-hierarchy-golden.test.ts` (hierarchy/selection/parenting, transforms, create/remove, named refusals, review, persistence, undo/redo, Play, and client parity), `tests/desktop/desktop-linux-seams.test.ts`, the spawned `--smoke` proof in CI (`.github/workflows/desktop-linux.yml`), and the recorded builds/pixel observations plus the offered workflow artifact in [`desktop-linux.md`](desktop-linux.md) / [`asset-ingestion.md`](asset-ingestion.md) |
