...
| `@sceneaxi/desktop-shell` | **R2** | `pnpm build && node apps/desktop-shell/bin/sceneaxi-desktop.mjs --help`; for standalone chrome, `… chrome > shell.html`; the packaged host activates New, Open, Save, Undo, Redo, Play, and Export Web through the existing menus/palette and the Ship rail, plus the contained project/asset browser, project hierarchy, ordered multi-select, parenting, selected-object transforms, and bounded local create/remove | `apps/desktop-shell/test/bin-smoke.test.ts`, `apps/desktop-shell/test/{product-loop,visual-model,visual-tokens,chrome}.test.ts`, `tests/e2e/desktop-product-loop-golden.test.ts`, `tests/e2e/desktop-hierarchy-golden.test.ts`, `tests/e2e/desktop-project-browser-golden.test.ts`, `tests/e2e/desktop-command-interactions-golden.test.ts`, `tests/e2e/desktop-web-export-golden.test.ts`, `tests/parity/shell-cli-parity.test.ts` + the browser record in [`engine-desktop-surface.md`](engine-desktop-surface.md) |
...
| `@sceneaxi/desktop-linux` (packaged Linux desktop app, ADR 0024) | **R2** | `cd desktop/linux && pnpm install && pnpm build && pnpm start`; first launch offers contained New/Open/Recent project lifecycle; a bound root lists its canonical `scene.json` and admitted manifest assets, opens supported selections, and keeps canonical rows immutable; Game/Web expose the native first-class asset picker and stable-id review reload; a bound project exposes hierarchy v1, ordered multi-select, explicit-policy parenting, primary-object translation/rotation/scale, and validated-local-artifact create/remove, each staged as one E1 proposal and written only on Save; Ship exports a deterministic contained static Web viewer plus Delivery Handoff without deploying; Assistant Build's Local route compiles and mounts a typed sculpt without a provider; the running host also publishes the permission-bound protocol-v1 local CLI bridge; distributable via `pnpm dist` (AppImage + `.deb` + `SHA256SUMS`), proven by `pnpm smoke --packaged` | `tests/e2e/asset-pipeline-golden.test.ts`, `tests/desktop/desktop-project-browser.test.ts` and `tests/e2e/desktop-project-browser-golden.test.ts`, `tests/e2e/asset-ingestion-golden.test.ts` + `tests/desktop/desktop-asset-picker-host.test.ts`, `tests/e2e/desktop-web-export-golden.test.ts`, `tests/e2e/desktop-project-lifecycle-golden.test.ts`, `tests/desktop/desktop-project-lifecycle.test.ts`, `tests/e2e/desktop-linux-bridge-golden.test.ts`, `tests/e2e/desktop-cli-local-bridge-golden.test.ts`, `tests/desktop/desktop-scene-property.test.ts`, `tests/e2e/desktop-scene-property-golden.test.ts`, `tests/e2e/desktop-hierarchy-golden.test.ts`, `tests/desktop/desktop-linux-seams.test.ts`, the spawned `--smoke` proof in CI (`.github/workflows/desktop-linux.yml`), and the recorded builds/pixel observations plus the offered workflow artifact in [`desktop-linux.md`](desktop-linux.md) / [`asset-ingestion.md`](asset-ingestion.md) |
...
| Importers + plugin host | **R1** | `pnpm test:golden` | `tests/e2e/asset-pipeline-golden.test.ts` (all bounded manifest families, previews, refusals, stable-id hot reload, browser/assistant/CLI parity), `tests/e2e/asset-ingestion-golden.test.ts` (preserved contained GLB/glTF desktop/CLI parity and Three projection), `tests/e2e/importers-plugin-golden.test.ts`, `tests/e2e/plugin-capability-golden.test.ts` |
...
The separate `asset import` and `asset reload` verbs stage only the bounded
manifest profiles in [`asset-ingestion.md`](asset-ingestion.md); they do not broaden or rewrite the
