...
| `@sceneaxi/desktop-linux` (packaged Linux desktop app, ADR 0024) | **R2** | `cd desktop/linux && pnpm install && pnpm build && pnpm start`; first launch offers contained New/Open/Recent lifecycle; a bound root lists `scene.json` and admitted manifest assets; Game/Web expose the native first-class asset picker and stable-id review reload; selected-instance edits save through E1; Ship exports the deterministic contained Web viewer; Local Assistant Build remains provider-free; the running host publishes the permission-bound local CLI bridge; distributable via `pnpm dist`, subject to the recorded Electron 43.2.0 host limitation | `tests/e2e/asset-pipeline-golden.test.ts`, project-browser and asset-ingestion goldens, Web-export golden, lifecycle/scene/bridge suites, `tests/desktop/desktop-linux-seams.test.ts`, and the packaged CI smoke |
