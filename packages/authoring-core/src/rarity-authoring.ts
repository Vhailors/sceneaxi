 ...
 *
 * It closes over no module binding on purpose: the Engine Desktop chrome embeds
 * this exact function into its emitted browser script with `String()`, the way
 * the staging decision already does, so the packaged renderer and the chrome
 * cannot drift into two paraphrases. It validates the value it is handed and
 * answers `null` rather than printing a partially-known descriptor.
 */
