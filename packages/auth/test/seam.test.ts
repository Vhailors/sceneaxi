...
import { AUTH_REFUSE_REASONS, seam } from "@sceneaxi/auth";

const manifest = JSON.parse(
  readFileSync(new URL("../package.json", import.meta.url), "utf8"),
) as { name: string; sceneaxi: { releaseGroup: string } };

describe("@sceneaxi/auth public seam", () => {
  ...
  it("freezes its refusal vocabulary with no duplicate reasons", () => {
    expect(Object.isFrozen(AUTH_REFUSE_REASONS)).toBe(true);
    const values = Object.values(AUTH_REFUSE_REASONS);
