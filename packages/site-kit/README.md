...
| `desktop-app-offer.ts` | the validated Linux workflow-artifact offer for umbrella `/engine` — exact run, source commit, files, sizes, and checksums held in lockstep with [`docs/desktop-linux.md`](../../docs/desktop-linux.md), with named refusals for missing or broken metadata |
