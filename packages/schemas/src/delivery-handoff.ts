import { parseUnambiguousJson } from "./unambiguous-json.js";
...
export function parseDeliveryHandoffText(
  ...
  if (!parsed.ok && parsed.code === "parse-error") {
    return refuse(
      "parse-error",
      "$",
      `Delivery handoff JSON parse failed: ${parsed.message}`,
    );
  }
  if (!parsed.ok) {
    return refuse(
  ...
  return validateDeliveryHandoff(parsed.value);
}
